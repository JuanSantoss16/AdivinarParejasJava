...
public class AdivinarParejas extends JFrame implements ActionListener {
		...
		for (int i = 0; i < 4; i++) { // Tratamos las columnas
			for (int j = 0; j < 4; j++) {// Tratamos las filas
				JButton btn = new JButton("", new ImageIcon(
						this.getClass().getResource("fondo.gif")));
				...
				JLabel label2 = new JLabel(
						"© Juego Adivinar Parejas - Alberto Arrojo Carrasco");
	...
	public void actionPerformed(ActionEvent e) {
		...
		for (int i = 0; i < arrayBotones.length; i++) {
			...
			if (arrayCartas[i].btn == e.getSource()
					&& arrayCartas[i].revelada == false) {
				arrayCartas[i].btn.setIcon(arrayCartas[i].img);
				
				if (par == 0) {

					arrayCartas[i].revelada = true;
					arrayCartaTemporal = arrayCartas[i];
					...
					if (arrayCartas[i].valor == arrayCartaTemporal.valor) {

						arrayCartas[i].revelada = true;

						boolean bandera = true;

						for (Carta elemento : arrayCartas) {

							if (elemento.revelada == false) {
						...
						if (bandera) {
							arrayCartas[i].btn
									.update(arrayCartas[i].btn.getGraphics());
							sonidoGanar();
							Icon iconoGanador = new ImageIcon(
									getClass().getResource("ganador.png"));
							JOptionPane.showMessageDialog(this,
									"¡Felicidades! Has conseguido " + puntos
											+ " puntos",
							...
							int respuesta = insertar(con, "puntos",
							...
							if (respuesta == 0) {
								...
								System.out
										.println("Error en el alta de jugador");
							}
							desconectar(con);
	...
	public void tapar(int a) { // Método para volver a tapar la carta
		...
		arrayCartas[a].btn.setIcon(
				new ImageIcon(this.getClass().getResource("fondo.gif")));
		arrayCartas[Integer
				.valueOf(arrayCartaTemporal.btn.getName())].revelada = false;
	...
	public int insertar(Connection con, String tabla, String nombreJugador,
			int puntos) { // Insertar en la base de
		// datos
		int respuesta = 0;
		try {
